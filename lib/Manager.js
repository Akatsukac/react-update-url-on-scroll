...
var Manager = function Manager() {
  ...
  this.getBasePath = function (anchors) {
    var newBasePath = typeof window !== 'undefined' ? ('' + window.location.origin + window.location.pathname).replace(/\/$/, '') : '';
  ...
  this.handleScroll = function () {
    ...
    if (nextAnchor && prevAnchor !== nextAnchor) {
      ...
      console.log('we forced a hash', _this.anchors[nextAnchor]);
      (0, _hash.updateHash)({
  ...
  this.handleHashChange = function (e) {
    ...
    if (_this.forcedHash) {
      ...
      if (typeof window !== 'undefined' && _this.config.scrollOnImagesLoad && !_this.imagesAreLoaded) {
  ...
  this.goToSection = function (id) {
    ...
    var element = typeof document !== 'undefined' ? (_this.anchors[id] ? _this.anchors[id].component : null) || document.getElementById(id) : '';
    ...
    if (element) {
      setTimeout(function () {
        if (typeof window !== 'undefined') {
          var _marginTop = ~~(element.currentStyle || window.getComputedStyle(element).marginTop.replace(/\D+/g, ''));
        }
  ...
  setTimeout(function () {
    ...
    if (_this.config.scrollOnImagesLoad) {
      ...
      var imgs = typeof document !== 'undefined' ? document.images : '';
      var len = imgs.length;
      var counter = 0;
